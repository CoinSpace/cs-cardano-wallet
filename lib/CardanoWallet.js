...
export default class CardanoWallet extends CsWallet {
  ...
  get dummyExchangeDepositAddress() {
    if (this.development) {
      ...
      return 'addr_test1qrh3krck9739m5wu2krhszaj8evm7hzu43qsc8rnplju7vk495gs9vvyl5cj0yap8aghg2sfwsldpfpkne5ssksyjeuqzqugfm';
    } else {
      return 'addr1qx3vxt9dzs77swltjuquc8tkl5hsmaezmvcsuusf7cpfm7uga5gvsdpcmct8ecf4clufjqaeyw5vlyc5v5qk96mmmegqnqwpe8';
    }
  }
  ...
  async validateAmount({ address, amount, price }) {
    ...
    if (value > maxAmount) {
      const unconfirmedMaxAmount = await this.#estimateMaxAmount({ price }, true);
  ...
  async _estimateMaxAmount({ price }, unconfirmed = false) {
  ...
  async _estimateTransactionFee({ address, value, price }) {
  ...
  async loadTransactions({ cursor = 0 } = {}) {
    ...
    const transactions = await Promise.all(this.#transformTxs(txs));
  ...
  async loadTransaction(id) {
    if (this.#transactions.has(id)) {
      ...
      try {
        return this.#transformTx(await this.#api.transaction(id));
      } catch (err) {
        return;
      }
    }
