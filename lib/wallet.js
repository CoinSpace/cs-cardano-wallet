...
export default class CardanoWallet extends CsWallet {
  ...
  async load() {
    ...
    const utxos = await this.#addressesUtxos(this.address);
    this.#utxos.push(...utxos.map((utxo) => {
      return {
        ...utxo,
        txId: utxo.txHash,
    ...
    this.#dustThreshold = dustThreshold > ADA ? dustThreshold : ADA;
  ...
  async _estimateTransactionFee({ address, value }) {
    ...
    const maxAmount = await this.#estimateMaxAmount();
    const hasChange = (maxAmount - value) >= this.#dustThreshold;
